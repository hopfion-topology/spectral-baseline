...
The untwisted baseline ζ₀(2) is normalized so that the bulk contribution recovers Vol(S³ × S¹) = 4π³ ≈ 124.025. The golden-angle twist (θ = 1/φ²) produces a normalized correction of approximately -6.6 × 10⁻⁵. This correction is more than five orders of magnitude below the required irrational gain π² + π ≈ 13.01.
...
The S¹ fiber shift `n*theta` is reduced modulo one before the finite m-sum is taken, since the infinite sum depends only on the fractional part of the shift.
...
Multiple decompositions were tested (uniform, triangular, parity-stepped, SU(2) branching with half-integer charges). All give corrections well below the required irrational gain of 13.01. The conclusion is insensitive to the choice of `w(k,n)`.
